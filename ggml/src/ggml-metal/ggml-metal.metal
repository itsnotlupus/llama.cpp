...
void dequantize_iq1_xxxs(device const block_iq1_xxxs * xb, short il, thread type4x4 & reg) {
    ...
    const float d = xb->d;
    device const uint8_t * qs = xb->qs + 4*ib32 + 2*il;
    device const uint8_t * sc = xb->sc;
    const int nib = (sc[ib32/2] >> (4*(ib32&1))) & 0xf;
...
void dequantize_iq1_xxs(device const block_iq1_xxs * xb, short il, thread type4x4 & reg) {
    ...
    device const uint8_t * qs = xb->qs + 4*ib32 + 2*il;
    device const uint8_t * qh = xb->qh;
    const float dl = d * (2*((qh[ib32] >> 4) & 7) + 1);
    const float ml = dl * (qh[ib32] & 0x80 ? -1 - IQ1S_DELTA : -1 + IQ1S_DELTA);
    constant uint8_t * grid1 = (constant uint8_t *)(iq1_xxs_grid_gpu + (qs[0] | (((qh[ib32] >> 2*il+0) & 1) << 8)));
    constant uint8_t * grid2 = (constant uint8_t *)(iq1_xxs_grid_gpu + (qs[1] | (((qh[ib32] >> 2*il+1) & 1) << 8)));
    for (int i = 0; i < 4; ++i) {
        reg[0][i] = dl * (grid1[i] & 0xf) + ml;
        reg[1][i] = dl * (grid1[i] >>  4) + ml;
        reg[2][i] = dl * (grid2[i] & 0xf) + ml;
...
void dequantize_iq1_xs(device const block_iq1_xs * xb, short il, thread type4x4 & reg) {
    ...
    const float dl = d * (2*(nib & 7) + 1);
    const float ml = dl * (nib & 8 ? -1 - IQ1S_DELTA : -1 + IQ1S_DELTA);
    constant uint8_t * grid1 = (constant uint8_t *)(iq1_xs_grid_gpu + (qs[0] | (((qh[ib32] >> 4*il+0) & 3) << 8)));
    constant uint8_t * grid2 = (constant uint8_t *)(iq1_xs_grid_gpu + (qs[1] | (((qh[ib32] >> 4*il+2) & 3) << 8)));
    for (int i = 0; i < 4; ++i) {
        reg[0][i] = dl * (grid1[i] & 0xf) + ml;
        reg[1][i] = dl * (grid1[i] >>  4) + ml;
        reg[2][i] = dl * (grid2[i] & 0xf) + ml;
...
void kernel_mul_mv_iq1_xxs_f32_impl(
    ...
    for (int ib32 = ix; ib32 < nb32; ib32 += 32) {
        ...
        for (short row = 0; row < nr0; row++) {
            constant uint8_t * grid1 = (constant uint8_t *)(iq1_xxs_grid_gpu + (qs[0] | (((qh[0] >> 0) & 1) << 8)));
            constant uint8_t * grid2 = (constant uint8_t *)(iq1_xxs_grid_gpu + (qs[1] | (((qh[0] >> 1) & 1) << 8)));
            constant uint8_t * grid3 = (constant uint8_t *)(iq1_xxs_grid_gpu + (qs[2] | (((qh[0] >> 2) & 1) << 8)));
            constant uint8_t * grid4 = (constant uint8_t *)(iq1_xxs_grid_gpu + (qs[3] | (((qh[0] >> 3) & 1) << 8)));
...
void kernel_mul_mv_iq1_xs_f32_impl(
    ...
    for (int ib32 = ix; ib32 < nb32; ib32 += 32) {
        ...
        for (short row = 0; row < nr0; row++) {
            ...
            constant uint8_t * grid1 = (constant uint8_t *)(iq1_xs_grid_gpu + (qs[0] | (((qh[0] >> 0) & 3) << 8)));
            constant uint8_t * grid2 = (constant uint8_t *)(iq1_xs_grid_gpu + (qs[1] | (((qh[0] >> 2) & 3) << 8)));
            constant uint8_t * grid3 = (constant uint8_t *)(iq1_xs_grid_gpu + (qs[2] | (((qh[0] >> 4) & 3) << 8)));
            constant uint8_t * grid4 = (constant uint8_t *)(iq1_xs_grid_gpu + (qs[3] | (((qh[0] >> 6) & 3) << 8)));
